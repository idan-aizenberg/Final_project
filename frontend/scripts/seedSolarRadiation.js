/**
 * Upload solar_radiation data using bulk RPC (fast method)
 * Run with: node scripts/seedSolarRadiation.js
 * 
 * Prerequisites:
 * - Run frontend/supabase-add-solar-radiation-column.sql
 * - Run frontend/supabase-bulk-update-solar-radiation-function.sql
...
/**
 * Check which days already have solar radiation data
 */
async function checkProgress() {
  ...
  for (const day of checkDays) {
    const { count: withSolar } = await supabase
      .from('weather_forecasts')
      .select('*', { count: 'exact', head: true })
      .eq('forecast_year', 2025)
      .eq('day_of_year', day)
      .not('solar_radiation', 'is', null);
    ...
    console.log(`  checked day ${day}: ${withSolar || 0}/${total || 0}`);
    
    if ((withSolar || 0) < (total || 0)) {
...
async function uploadSolarData() {
  console.log('🚀 Starting solar radiation data upload...\n');
  ...
  if (missingDays.length === 0) {
    console.log('\n✅ All sampled days already have solar radiation data!');
    console.log('Proceeding with full upload to ensure complete coverage...\n');
  }

  // Read CSV
  const csvPath = path.join(__dirname, '..', '..', 'data', 'data_integr_2025_10.csv');
  ...
  for (let lineIdx = 1; lineIdx < lines.length; lineIdx++) {
    ...
    for (let i = 1; i < values.length; i++) {
      const solar = parseFloat(values[i]);
      if (isNaN(solar)) continue;

      const gridIndex = i - 1;
      gridIndices.push(gridIndex);
      solarValues.push(solar);
      lastDayValues.push({ gridIndex, solar });
    }

    // Call bulk RPC to update this day
    const runUpdate = async () => {
      return supabase.rpc('bulk_update_solar_radiation', {
        p_day: dayOfYear,
        p_grid_indices: gridIndices,
        p_solar_values: solarValues,
        p_year: 2025
      });
    };
  ...
  if (lastDay < 365 && lastDayValues.length > 0) {
    ...
    for (let day = lastDay + 1; day <= 365; day++) {
      ...
      const solarValues = lastDayValues.map(v => v.solar);

      try {
        const { data: result, error } = await supabase.rpc('bulk_update_solar_radiation', {
          p_day: day,
          p_grid_indices: gridIndices,
          p_solar_values: solarValues,
          p_year: 2025

/**
 * Upload wind_speed_u_max data using bulk RPC (fast method)
 * Run with: node scripts/seedWindSpeedU.js
 * 
 * Prerequisites:
 * - Run frontend/supabase-add-wind-speed-u-column.sql
 * - Run frontend/supabase-bulk-update-wind-speed-u-function.sql
...
/**
 * Check which days already have wind_speed_u_max data
 */
async function checkProgress() {
  console.log('🔎 Checking current DB progress for wind_speed_u_max...');
  
  const checkDays = [25, 50, 75, 100, 125, 150, 175, 200, 225, 250, 275, 300, 325];
  const missingDays = [];
  
  for (const day of checkDays) {
    const { count: withWindU } = await supabase
      .from('weather_forecasts')
      .select('*', { count: 'exact', head: true })
      .eq('forecast_year', 2025)
      .eq('day_of_year', day)
      .not('wind_speed_u_max', 'is', null);
    ...
    console.log(`  checked day ${day}: ${withWindU || 0}/${total || 0}`);
    
    if ((withWindU || 0) < (total || 0)) {
 ...
 */
async function uploadWindSpeedUData() {
  console.log('🚀 Starting wind_speed_u_max data upload...\n');
  ...
  if (missingDays.length === 0) {
    console.log('\n✅ All sampled days already have wind_speed_u_max data!');
    console.log('Proceeding with full upload to ensure complete coverage...\n');
  }

  // Read CSV - data_integr_2025_16.csv contains wind_speed_u_MAX
  const csvPath = path.join(__dirname, '..', '..', 'data', 'data_integr_2025_16.csv');
  ...
  for (let lineIdx = 1; lineIdx < lines.length; lineIdx++) {
    ...
    for (let i = 1; i < values.length; i++) {
      const windU = parseFloat(values[i]);
      if (isNaN(windU)) continue;

      const gridIndex = i - 1;
      gridIndices.push(gridIndex);
      windUValues.push(windU);
      lastDayValues.push({ gridIndex, windU });
    }

    // Call bulk RPC to update this day
    const runUpdate = async () => {
      return supabase.rpc('bulk_update_wind_speed_u', {
        p_day: dayOfYear,
        p_grid_indices: gridIndices,
        p_wind_u_values: windUValues,
        p_year: 2025
      });
    };
  ...
  if (lastDay < 365 && lastDayValues.length > 0) {
    ...
    for (let day = lastDay + 1; day <= 365; day++) {
      ...
      const windUValues = lastDayValues.map(v => v.windU);

      try {
        const { data: result, error } = await supabase.rpc('bulk_update_wind_speed_u', {
          p_day: day,
          p_grid_indices: gridIndices,
          p_wind_u_values: windUValues,
          p_year: 2025
...
// Run
uploadWindSpeedUData().catch(console.error);
